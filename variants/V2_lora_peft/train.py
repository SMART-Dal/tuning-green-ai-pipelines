...
# 2. Dataset helpers
def load_bigvul():
    ds = load_dataset("bstee615/bigvul")  # splits hosted on HF Hub
...
def prep_dataset(dataset, tok, text_col, label_col, max_len):
    ...
    ds_tok = dataset.map(tok_fn,
                         remove_columns=[c for c in dataset.column_names if c not in keep],
                         batched=False)
...
def main(cfg_path: Path, output_root: Path):
    ...
    run_name = f"{variant}_{datetime.now():%Y%m%d_%H%M%S}" # Gets the variant folder name (e.g., V2_lora_peft)
