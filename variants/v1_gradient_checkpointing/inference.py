...
def main(cfg_path: Path, output_root: Path):
    ...
    run_name = f"{variant}_{datetime.now():%Y%m%d_%H%M%S}" # Gets the variant folder name (e.g., V1_gradient_checkpointing)
    ...
    try:
        ...
        with open(output_root / run_name / "energy_stats_inference.json", "w") as f:
            json.dump(tracker.final_emissions_data.toJSON(), f, indent=2)
