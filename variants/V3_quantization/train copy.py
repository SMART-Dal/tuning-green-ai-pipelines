...
# 2.  Dataset helpers
def load_bigvul():
    ds = load_dataset("bstee615/bigvul")          # train / validation / test
...
def main(cfg_path: Path, output_root: Path):
    ...
    run_name = f"{variant}_{datetime.now():%Y%m%d_%H%M%S}" # Gets the variant folder name (e.g., V3_quantization)
    ...
    try:
        ...
        model, tokenizer = FastModel.from_pretrained(
            ...
            load_in_4bit=True,  # Changed from True to False
            # load_in_8bit = True,
            max_seq_length=vcfg.max_length,
        ...
        training_args = TrainingArguments(
            output_dir          = output_root / run_name / "ckpt",
